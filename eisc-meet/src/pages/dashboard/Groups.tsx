...
const Groups = () => {
  ...
  useEffect(() => {
    ...
    connectSocket().then((connected) => {
      if (connected) socket.emit("room:occupancy:watch", { roomIds });
    });

    const handleOccupancySnapshot = (payload: { rooms?: Record<string, number> }) => {
      setRoomOccupancy(payload.rooms ?? {});
    };

    const handleOccupancy = (payload: { roomId?: string; count?: number }) => {
      if (!payload.roomId || typeof payload.count !== "number") return;

      setRoomOccupancy((current) => ({
        ...current,
        [payload.roomId as string]: payload.count as number,
      }));
    };

    socket.on("room:occupancy:snapshot", handleOccupancySnapshot);
    socket.on("room:occupancy", handleOccupancy);

    return () => {
      socket.off("room:occupancy:snapshot", handleOccupancySnapshot);
      socket.off("room:occupancy", handleOccupancy);
    };
  }, [authUser?.uid, rooms]);
            ...
            {loadingRooms ? (
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {filteredRooms.map((room) => {
                  ...
                  const currentParticipants = roomOccupancy[room.id] ?? 0;
